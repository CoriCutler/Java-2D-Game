...
public class dockTile extends Tile {
	...
	public dockTile(SpriteSheet sheet) {
		super(sheet);
	}
	
	public void render(int x, int y, Screen screen) {
		// top left
				if (Level.getTile(x + 1, y) == Tile.dockTile
						&& Level.getTile(x - 1, y) == Tile.waterTile
						&& Level.getTile(x, y + 1) == Tile.dockTile
						&& Level.getTile(x, y - 1) == Tile.waterTile)
					screen.renderTile(x << 5, y << 5, sheet.getSprite()[0]);
				// top
				else if (Level.getTile(x + 1, y) == Tile.dockTile
						&& Level.getTile(x - 1, y) == Tile.dockTile
						&& Level.getTile(x, y + 1) == Tile.dockTile
						&& Level.getTile(x, y - 1) == Tile.waterTile)
				...
				else screen.renderTile(x << 5, y << 5, sheet.getSprite()[5]);
			}
}
