...
public class grass_stoneTile extends Tile {

	public grass_stoneTile(SpriteSheet sheet) {
		super(sheet);
	}

	public void render(int x, int y, Screen screen) {
		// top left
		if (Level.getTile(x + 1, y) == Tile.grass_stoneTile
				&& Level.getTile(x - 1, y) == Tile.grassTile
				&& Level.getTile(x, y + 1) == Tile.grass_stoneTile
				&& Level.getTile(x, y - 1) == Tile.grassTile)
			screen.renderTile(x << 5, y << 5, sheet.getSprite()[0]);
		// top
		else if (Level.getTile(x + 1, y) == Tile.grass_stoneTile
				&& Level.getTile(x - 1, y) == Tile.grass_stoneTile
				&& Level.getTile(x, y + 1) == Tile.grass_stoneTile
				&& Level.getTile(x, y - 1) == Tile.grassTile)
		...
		// bottom right
			...
			screen.renderTile(x << 5, y << 5, sheet.getSprite()[8]);
				...
				&& Level.getTile(x, y + 1) == Tile.grass_stoneTile
				&& Level.getTile(x, y - 1) == Tile.grass_stoneTile
				&& Level.getTile(x - 1, y - 1) == Tile.grass_stoneTile
				&& Level.getTile(x + 1, y - 1) == Tile.grass_stoneTile
				&& Level.getTile(x - 1, y + 1) == Tile.grass_stoneTile
